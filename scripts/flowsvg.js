...
var flowSVG = (function () {
  ...
  function arrowLine() {
    var group = draw.group(),
        al = arrowHead(group).move(-(config.arrowHeadHeight / 2),
                                    config.connectorLength - config.arrowHeadHeight),
        line = draw.line(0, 0, 0, config.connectorLength - config.arrowHeadHeight)
                   .attr({ 'stroke-width':  config.connectorStrokeWidth,
                           'stroke': config.connectorStrokeColour
                        });

    group.add(line).add(al);

    return group;
  }

  function arrowHead(g) {
    var coords = "0,0 " + config.arrowHeadHeight
                        + ",0 "
                        + config.arrowHeadHeight / 2
                        + ","
                        + config.arrowHeadHeight,  
        ah = g.polygon(coords)
              .fill({color: config.arrowHeadColor, opacity: config.arrowHeadOpacity})
  ...
  function lineLabel(t, g) {
    var text, labelGroup = g.group(),
        label = labelGroup.rect(config.labelWidth, config.labelHeight)
                          .radius(config.labelRadius)
                          .stroke({ width: config.labelStroke })
                          .attr({ opacity: config.labelOpacity,
                                  fill: config.labelFill
                                });

    label.move( -(config.labelWidth / 2), config.labelHeight / 2 );

    text = labelGroup.text(t)
                     .attr({ fill: config.labelTextColour,
                            'text-anchor': 'middle',
  ...
  function decision(options) {
    ...
    //              .attr({ fill: config.decisionFill,
    //                      "class": 'fc-rhombus'
    //                   });

    // my code starts ----- for rectangle shape.
    shape = group.rect(config.startWidth, config.decisionHeight/2)
                 .attr({ fill: config.decisionFill });
    // my code ends
  ...
  function finish(options) {
    var tip,
        ...
        tiptxt,
        group = chartGroup.group()
                          .attr({ "class": "finish-group" }),
        rect = group.rect(config.finishWidth, config.finishHeight)
                    .attr({ fill: config.finishFill,
                            "class": "fc-finish" })
                    .radius(20),
        content = group.group();
  ...
  function process(options) {
    var group = chartGroup.group()
        ...
        rect = group.rect(config.processWidth, config.processHeight)
                    .attr({ fill: config.processFill,
    ...
    if(interactive === true) {
      ...
      group.on('click', function () {
        ...
        if(shapes[0].show === false) {
          shapes[0].show = true;
          // mycode starts
          lowerConnector.animate().opacity(config.maxOpacity);
          lowerConnector.attr('visibility', 'visible');
          // my code ends
          firstShape.animate().opacity(config.maxOpacity);
          firstShape.attr('visibility', 'visible');
        } else {
          // mycode starts
          lowerConnector.animate().opacity(config.minOpacity);
          lowerConnector.attr('visibility', 'hidden');
          // mycode ends
          firstShape.animate().opacity(config.minOpacity);
          firstShape.attr('visibility', 'hidden');
          shapes[0].show = false;

          // PG_DOUBT:- WHAT IS THIS CODE FOR?
  ...
  function buttonBar() {
    ...
    activeBtn = btnGroup.rect(btnBarWidth, config.btnBarHeight)
                        .fill(config.startFill)
                        .radius(config.btnBarRadius);

    staticBtn = btnGroup.rect(btnBarWidth, config.btnBarHeight)
                        .fill(config.startFill)
                        .radius(config.btnBarRadius)
                        .move(btnBarWidth + 5, 0);

    btnGroup.text('Interactive')
            .attr({ 'fill' : config.startTextColour,
                    'font-size': config.btnBarFontSize,
                    'pointer-events': 'none' })
            .cy(config.btnBarHeight / 2)
            .cx(btnBarWidth / 2);

    btnGroup.text('Static')
            .attr({ 'fill' : config.startTextColour,
                    'font-size':  config.btnBarFontSize,
                    'pointer-events': 'none' })
            .cy(config.btnBarHeight / 2)
  ...
  function addConnectors(element) {
    ...
    if(element.yesid) {
      ...
      element.svgid.polyline(angleLine(startln, endln, element))
                   .stroke({ width: config.connectorStrokeWidth, color: config.connectorStrokeColour})
                   .fill('none');
    }
  ...
  //     element.conngroup.polyline(angleLine(startln, endln, element))
  //                      .stroke({ width:  config.connectorStrokeWidth, color: config.connectorStrokeColour})
  //                      .fill('none');
  //   }
  ...
  function angleLine(start, end, element) {
    var e = element.svgshape,
        p1, p2, p3, p4, p5, 
        spacer = config.arrowHeadHeight * 2, endPos;
